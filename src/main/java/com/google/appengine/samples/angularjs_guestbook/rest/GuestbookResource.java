...
public class GuestbookResource {
	...
	private List<Greeting> getGreetings(String guestbookName) {
		...
		List<Entity> greetingEntities = datastoreService.prepare(query).asList(FetchOptions.Builder.withLimit(10));
		for (Entity greeting : greetingEntities) {
			greetings.add(Greeting.fromEntity(greeting));
		}
		logger.info(greetingEntities.toString());
		return greetings;
	}
	...
	public GuestbookResponse getGuestbookData(@DefaultValue("default") @PathParam("guestbookName") final String guestbookName) 
											 throws Exception {
		return new GuestbookResponse(guestbookName, getGreetings(guestbookName), UserServiceInfo.get("/"));
	}
